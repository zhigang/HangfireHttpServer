...
namespace HangfireHttpServer
{
    public static class HangfireCollectionExtensions
    {
            ...
            IGlobalConfiguration globalConfiguration)
        {
            ...
            {
                var cultureName = langStr.Replace(".UTF-8", "").Replace("_", "-");
                System.Threading.Thread.CurrentThread.CurrentUICulture = new System.Globalization.CultureInfo(cultureName);
            }

            var hangfireSettings = serverProvider.GetRequiredService<IOptions<HangfireSettings>>().Value;
            ConfigFromEnv(hangfireSettings);

            var httpJobOptions = serverProvider.GetRequiredService<IOptions<HangfireHttpJobOptions>>().Value;
            ...
            if (string.IsNullOrEmpty(sqlConnectStr))
            {
                throw new ArgumentNullException(nameof(sqlConnectStr), "Hangfire PostgreSQL connection string cannot be null or empty.");
            }

            globalConfiguration
                .UsePostgreSqlStorage(options => options.UseNpgsqlConnection(sqlConnectStr))
        ...
        public static IApplicationBuilder ConfigureSelfHangfire(this IApplicationBuilder app, IConfiguration configuration)
        {
            ...
            var hangfireSettings = services.GetRequiredService<IOptions<HangfireSettings>>().Value;
    ...
    public class HangfireSettings
    {
        ...
        public required List<string> JobQueues { get; set; }
